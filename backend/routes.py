...
# Mensagens (Chat) - RF09: comunicação entre cliente e cuidador durante a solicitação/prestação do serviço

mensagem_bp = Blueprint('mensagem_bp', __name__)

mensagem_bp.add_url_rule('/mensagens', view_func=MensagemController.criar, methods=['POST'])
mensagem_bp.add_url_rule('/contratos/<int:idContrato>/mensagens', view_func=MensagemController.listar_por_contrato, methods=['GET'])
mensagem_bp.add_url_rule('/contratos/<int:idContrato>/mensagens/lidas', view_func=MensagemController.marcar_lidas, methods=['PATCH'])
mensagem_bp.add_url_rule('/mensagens/<int:idMensagem>', view_func=MensagemController.deletar, methods=['DELETE'])
# Lista as conversas (uma por contrato) de um usuário, com resumo da última mensagem e não lidas
mensagem_bp.add_url_rule('/usuarios/<int:idUsuario>/conversas', view_func=MensagemController.listar_conversas, methods=['GET'])
